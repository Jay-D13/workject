...
export function ProjectCard({ project, onClick, mode = 'grid' }: ProjectCardProps) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'retro-panel group flex h-full w-full flex-col p-6 text-left transition-all duration-200',
        'hover:-translate-y-0.5 hover:border-border-hover',
    ...
    >
      ...
      <div className="mt-auto">
        ...
        <div className="flex items-center justify-between border-t-2 border-border/55 pt-4">
          <span className="text-xs font-medium text-text-muted">
            {new Date(project.updatedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
          </span>
          {project.repoUrl && (
            <span className="retro-inset p-1.5 text-text-muted transition-colors group-hover:border-border-hover group-hover:text-text-primary">
